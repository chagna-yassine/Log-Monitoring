"""
Upload Wardbeck AIT logs to Hugging Face as a Dataset object.
This script processes the Wardbeck dataset from Zenodo.
"""
...
def main():
    """Upload Wardbeck AIT logs to Hugging Face."""
    print("="*70)
    print("UPLOAD WARDBECK AIT LOGS TO HUGGING FACE")
    print("="*70)
    ...
    print("\nTo upload Wardbeck logs to Hugging Face, you need:")
    print("1. A Hugging Face token (get from: https://huggingface.co/settings/tokens)")
    print("2. A repository name (e.g., 'username/ait-wardbeck-raw-logs')")
    print()
    ...
    repo_name = input("\nEnter Hugging Face repository name (e.g., 'username/ait-wardbeck-raw-logs'): ").strip()
    if not repo_name:
    ...
    if not gather_dir.exists():
        ...
        print("Please extract the Wardbeck dataset to: datasets/ait")
        print("Or make sure the dataset is extracted in: datasets/ait/gather")
        print("Download from: https://zenodo.org/records/5789064/files/wardbeck.zip")
        sys.exit(1)
    ...
    csv_file = output_path / "wardbeck_raw_logs_dataset.csv"
