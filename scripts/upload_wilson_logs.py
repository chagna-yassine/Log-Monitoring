"""
Upload Wilson AIT logs to Hugging Face as a Dataset object.
This script processes the Wilson dataset from Zenodo.
"""
...
def main():
    """Upload Wilson AIT logs to Hugging Face."""
    print("="*70)
    print("UPLOAD WILSON AIT LOGS TO HUGGING FACE")
    print("="*70)
    ...
    print("\nTo upload Wilson logs to Hugging Face, you need:")
    print("1. A Hugging Face token (get from: https://huggingface.co/settings/tokens)")
    print("2. A repository name (e.g., 'username/ait-wilson-raw-logs')")
    print()
    ...
    repo_name = input("\nEnter Hugging Face repository name (e.g., 'username/ait-wilson-raw-logs'): ").strip()
    if not repo_name:
    ...
    if not gather_dir.exists():
        ...
        print("Please extract the Wilson dataset to: datasets/ait")
        print("Or make sure the dataset is extracted in: datasets/ait/gather")
        print("Download from: https://zenodo.org/records/5789064/files/wilson.zip")
        sys.exit(1)
    ...
    csv_file = output_path / "wilson_raw_logs_dataset.csv"
