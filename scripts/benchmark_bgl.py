"""
BGL Benchmark Script

Runs the complete benchmarking pipeline for log anomaly detection on BGL dataset.
"""
...
def main():
    """Main function to run BGL benchmarking."""
    print("="*70)
    print("LOG ANOMALY DETECTION MODEL BENCHMARKING - BGL DATASET")
    print("="*70)
    ...
    test_csv = output_path / "bgl_test.csv"
    ...
    if not test_csv.exists():
        print(f"Error: BGL test file not found: {test_csv}")
        print("Please run preprocess_bgl.py first.")
        sys.exit(1)
    
    print(f"BGL test file: {test_csv}")
    
    # Load test data
    print("\n" + "="*70)
    print("LOADING BGL TEST DATA")
    print("="*70)
    
    test_df = pd.read_csv(test_csv)
    print(f"Loaded {len(test_df):,} BGL test samples")
    ...
    # Run inference
    print("\n" + "="*70)
    print("RUNNING INFERENCE ON BGL")
    print("="*70)
    ...
    print(f"BGL predictions saved to: {predictions_file}")
    ...
    print("\n" + "="*70)
    print(f"BGL Results saved to: {results_dir.absolute()}")
    print("="*70)
